...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('play_sessions', function (Blueprint $table) {
            // Jenis waktu bermain: per_hour (ada batas waktu) atau bebas (tanpa batas)
            $table->enum('session_type', ['per_hour', 'free_play'])
                ->default('free_play')
                ->after('status')
                ->comment('per_hour = ada batas waktu, free_play = tanpa batas');

            // Kapan sesi seharusnya berakhir — hanya diisi untuk mode per_hour
            // Digunakan untuk trigger time_up dan matikan TV via STB
            $table->timestamp('planned_end_at')
                ->nullable()
                ->after('session_type')
                ->comment('Batas waktu sesi (khusus per_hour). Null untuk mode bebas.');
