...
import { authApi, customerAuthApi, usersApi, customersApi } from "../../services/api";
...
export default function UserInfoCard({ user }) {
  ...
  useEffect(() => {
    if (isOpen && user) {
      setIsFetching(true);
      const fetchApi = user.role === "customer" ? customerAuthApi.me() : authApi.me();
      fetchApi
        .then((res) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (user.role === "customer") {
        await customersApi.update(user.id, payload);
      } else {
        await usersApi.update(user.id, payload);
      }

      useAuthStore.setState({ user: { ...user, name: fullName, phone: formData.phone } });
      closeModal();
    } catch (error) {
    ...
    <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
          <div className="flex w-20 h-20 overflow-hidden items-center justify-center bg-brand-500 border border-gray-200 rounded-full dark:border-gray-800 text-white text-3xl font-bold">
            {user?.avatar ? (
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <form className="flex flex-col" onSubmit={handleSave}>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              <Button size="sm" type="submit" disabled={isLoading || isFetching}>
