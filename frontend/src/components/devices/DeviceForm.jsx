...
export function DeviceForm({ device, onClose, onSuccess }) {
  const isEdit = !!device;
  ...
  const mutation = useMutation({
    mutationFn: () =>
      isEdit ? devicesApi.update(device.id, form) : devicesApi.create(form),
    onSuccess,
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field
        label="Serial number konsol"
        hint="Opsional — untuk identifikasi unit fisik"
      >
        <Input
          value={form.ps_sn}
          onChange={(e) => set("ps_sn", e.target.value)}
          placeholder="cth. CUH-1200A"
        />
      </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Merek / model TV">
            <Input
              ...
              placeholder="cth. Samsung 32"
            />
          </Field>
          <Field label="Serial number TV">
            <Input
              value={form.tv_sn}
              onChange={(e) => set("tv_sn", e.target.value)}
              placeholder="cth. SN12345678"
            />
          </Field>
          <Field label="IP address TV" hint="Format: 192.168.x.x">
            <Input
              value={form.tv_ip_address}
              onChange={(e) => set("tv_ip_address", e.target.value)}
              placeholder="cth. 192.168.1.101"
            />
          </Field>
          <Field label="MAC address TV" hint="Format: XX:XX:XX:XX:XX:XX">
            <Input
              value={form.tv_mac_address}
              onChange={(e) => set("tv_mac_address", e.target.value)}
