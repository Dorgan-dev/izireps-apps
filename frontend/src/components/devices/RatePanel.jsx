...
export function RatePanel({ device, onClose }) {
  ...
  const { data: rates, isLoading } = useQuery({
    ...
    queryFn: () => devicesApi.rates(device.id).then((r) => r.data.data),
  });

  const deleteMutation = useMutation({
    mutationFn: (rate) => devicesApi.delete(`/rates/${rate.id}`),
    onSuccess: () => {
    ...
    <div className="flex flex-col gap-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Tambah minimal satu tarif agar perangkat bisa digunakan"
        />
      ) : (
            ...
            >
              <div className="flex-1">
                <p className="font-medium text-gray-900 text-sm">
                  {formatRupiah(rate.price_per_hour)} / jam
                </p>
