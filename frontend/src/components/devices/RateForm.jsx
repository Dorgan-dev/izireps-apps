...
export function RateForm({ deviceId, rate, onClose, onSuccess }) {
  const isEdit = !!rate;
  const [pricePerHour, setPricePerHour] = useState(
    String(rate?.price_per_hour ?? ""),
  );
  const [effectiveFrom, setEffectiveFrom] = useState(
    rate?.effective_from?.slice(0, 5) ?? "",
  );
  const [effectiveUntil, setEffectiveUntil] = useState(
    rate?.effective_until?.slice(0, 5) ?? "",
  );
    ...
    <div className="flex flex-col gap-4">
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        ...
        <span className="text-sm text-gray-700">
          Tarif default (berlaku seharian)
        </span>
      </label>
      ...
      {!isDefault && (
        <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-2 text-xs text-amber-700">
          Tarif ini akan aktif pada rentang jam yang dipilih. Pastikan tidak
          tumpang tindih dengan tarif lain.
        </div>
      )}
      ...
      <div className="flex gap-3 justify-end pt-2 border-t border-gray-100">
        ...
        <Button
          ...
          disabled={
            !pricePerHour || (!isDefault && (!effectiveFrom || !effectiveUntil))
          }
