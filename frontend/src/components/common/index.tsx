...
export const Button = ({
  ...
  const base = 'inline-flex items-center justify-center gap-2 font-medium rounded-lg border transition-colors focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed'
  const variants = {
...
export const ConfirmDialog = ({
  ...
  confirmLabel = 'Konfirmasi', variant = 'warning', loading
}: ConfirmProps) => {
  const colors = {
    danger:  { icon: 'text-red-600 bg-red-50',   btn: 'danger' as const },
    warning: { icon: 'text-amber-600 bg-amber-50', btn: 'secondary' as const },
    info:    { icon: 'text-blue-600 bg-blue-50',  btn: 'primary' as const },
  }
  const c = colors[variant]
  return (
    <ModalUi isOpen={open} onClose={onClose} size="sm" showCloseButton={false}>
      <div className="flex flex-col items-center text-center">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${c.icon}`}>
        ...
        <div className="flex w-full gap-3">
          ...
          <Button variant={c.btn} loading={loading} className="flex-1" onClick={onConfirm}>{confirmLabel}</Button>
        </div>
      </div>
    </ModalUi>
  )
...
// ─── Form Field ───────────────────────────────────────────────────────────────
interface FieldProps { label: string; hint?: string; error?: string; children: React.ReactNode }
...
export const EmptyState = ({ icon, title, description }: { icon: string; title: string; description?: string }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    {description && <p className="text-sm text-gray-400 mt-1 dark:text-gray-400">{description}</p>}
  </div>
)
