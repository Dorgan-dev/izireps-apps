...
function BookingLateCountdown({ targetDate }) {
  ...
  if (rem <= 0)
    return <span className="text-red-600 font-medium text-sm">Kadaluarsa</span>;
  const m = Math.floor(rem / 60);
  const s = rem % 60;
  return (
    <span className="text-red-500 font-medium text-sm tabular-nums">
      Sisa grace period: {m}:{s.toString().padStart(2, "0")}
    </span>
  );
}

export default function WaitingBookingCard({ booking, onStart, isStarting }) {
  ...
  return (
    <div
      className={`bg-white border-2 rounded-2xl p-4 flex flex-col gap-3 
      ...
      {isLate && !isExpired && (
        <div className="bg-red-100 rounded-xl px-3 py-2 flex flex-col mt-1">
          <p className="text-xs text-red-700 font-medium mb-1">
            ⚠️ Pelanggan terlambat {">"}15 menit
          </p>
