...
export default function NewSessionModal({ onClose }) {
  ...
  const [sessionType, setSessionType] = useState("free_play");
  ...
  const { data: devices } = useQuery({
    queryKey: ["devices"],
    queryFn: () => deviceApi.list().then((r) => r.data.data),
  });
  ...
  const totalDurationMin =
    (parseInt(hours) || 1) * 60 + (parseInt(minutes) || 0);
  ...
  const mutation = useMutation({
    mutationFn: () =>
      sessionApi.startWalkIn({
        device_id: Number(deviceId),
        session_type: sessionType,
        duration_minutes:
          sessionType === "per_hour" ? totalDurationMin : undefined,
        customer:
          custName || custPhone
            ? { name: custName, phone: custPhone }
            : undefined,
        fnb_items: Object.entries(cart)
          .filter(([, q]) => q > 0)
        ...
        <div className="flex items-center gap-2">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${step >= s ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-400"}`}
              >
                {s}
              </div>
              <span
            ...
            <Field label="Pilih perangkat">
              ...
              {!availableDevices.length && (
                <p className="text-xs text-red-500 mt-1">
                  Tidak ada perangkat yang tersedia saat ini.
                </p>
              )}
            </Field>
                ...
                {[
                  {
                    value: "free_play",
                    label: "Bebas",
                    desc: "Tidak ada batas waktu",
                  },
                  {
                    value: "per_hour",
                    label: "Per Jam",
                    desc: "Tentukan durasi di awal",
                  },
                ].map((opt) => (
                  <button
                    key={opt.value}
            ...
            {sessionType === "per_hour" && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex flex-col gap-3">
                <p className="text-xs font-medium text-blue-700">
                  Tentukan durasi bermain (minimal 1 jam)
                </p>
                <Field label="Jam">
                  <Select
                    value={String(hours)}
                ...
                <p className="text-xs text-blue-600">
                  ...
                  <span className="font-medium">
                    {new Date(
                      Date.now() + totalDurationMin * 60000,
