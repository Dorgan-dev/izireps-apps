...
type ClockTimePickerProps = {
    id?: string;
    label?: string;
    value: string; // "HH:mm" format 24 jam
...
export default function ClockTimePicker({
    ...
    const handX = (r: number, a: number) => CENTER + r * Math.sin((a * Math.PI) / 180);
    const handY = (r: number, a: number) => CENTER - r * Math.cos((a * Math.PI) / 180);
        ...
        <>
            ...
            >
                ...
                <div className="flex items-center gap-1 select-none">
                    ...
                    <span className="text-3xl font-bold text-gray-400 animate-pulse">:</span>
                    <button
                        type="button"
                    ...
                    <span className="ml-1 text-xs font-semibold text-gray-400 dark:text-gray-500">WIB</span>
                </div>
                ...
                >
                    <circle cx={CENTER} cy={CENTER} r={RADIUS + 6} fill="currentColor" className="text-gray-100 dark:text-gray-800" />

                    {selecting === 'hour' && (
                        <circle cx={CENTER} cy={CENTER} r={INNER_RADIUS + 6} fill="currentColor" className="text-gray-200 dark:text-gray-700" />
                    )}
                    ...
                    {Array.from({ length: 60 }, (_, i) => {
                        ...
                        return (
                            <line
                                key={i}
                                x1={handX(r1, a)} y1={handY(r1, a)}
                                x2={handX(r2, a)} y2={handY(r2, a)}
                            ...
                            {outerHours.map((h, i) => {
                                ...
                                return (
                                    <text
                                        ...
                                        fill={blocked ? '#cbd5e1' : isSelected ? '#465fff' : '#475467'}
                                    >
                            ...
                            {innerHours.map((h, i) => {
                                ...
                                return (
                                    <text
                                        ...
                                        fontWeight={isSelected ? 'bold' : 'normal'}
                                        fill={blocked ? '#cbd5e1' : isSelected ? '#465fff' : '#94a3b8'}
                                    >
                                        {String(h).padStart(2, '0')}
                                    </text>
                                );
                    ...
                    {selecting === 'minute' && [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55].map((m, i) => {
                        const a = (i / 12) * 360;
                        const r = RADIUS - 16;
                        const nx = CENTER + r * Math.sin((a * Math.PI) / 180);
                        const ny = CENTER - r * Math.cos((a * Math.PI) / 180);
                        const isSelected = minute === m || (minute >= m && minute < m + 5 && i !== 0);
                        ...
                        return (
                            <text
                                ...
                                fill={exactSelected ? '#465fff' : isSelected ? '#7a8fff' : '#475467'}
                            >
                    ...
                    <line
                        x1={CENTER} y1={CENTER}
                        x2={handX(hour < 12 ? RADIUS - 26 : INNER_RADIUS - 24, hourAngle)}
                        y2={handY(hour < 12 ? RADIUS - 26 : INNER_RADIUS - 24, hourAngle)}
                        stroke="#465fff"
                        strokeWidth={4}
                        strokeLinecap="round"
                ...
                <div className="flex w-full gap-2">
                    <button
                        type="button"
                        onClick={() => setSelecting(s => s === 'hour' ? 'minute' : 'hour')}
    ...
    return (
        <div className="relative w-full">
            ...
            >
                ...
                <span className={`transition-colors ${isOpen ? 'text-brand-500' : 'text-gray-400 dark:text-gray-500'}`}>
