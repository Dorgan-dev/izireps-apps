...
export default function OwnerReports() {
  ...
  const load = () => {
    setIsLoading(true);
    reportsApi.summary({ from, to }).then((res) => setData(res.data)).finally(() => setIsLoading(false));
  };
    ...
    <div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-6">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <Button onClick={load} loading={isLoading}>Tampilkan</Button>
      ...
      {data && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'Total Pendapatan', value: formatRupiah(data.total_revenue ?? 0) },
            { label: 'Total Sesi', value: data.total_sessions ?? 0 },
            { label: 'Total Booking', value: data.total_bookings ?? 0 },
            { label: 'Pendapatan FnB', value: formatRupiah(data.fnb_revenue ?? 0) },
          ].map((card) => (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-gray-300 mb-4">Grafik Pendapatan per Hari</h2>
