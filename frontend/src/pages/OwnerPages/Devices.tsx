import { TbDeviceDesktop, TbDeviceDesktopCog, TbDeviceDesktopCode, TbDeviceDesktopCheck } from "react-icons/tb";
...
export default function Devices() {
  ...
  const filtered = allDevices.filter((d) => {
    const matchType =
      filterType === 'all' || d.ps_type === filterType
    const matchStatus =
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Metric title='Total unit' amount={counts.total} icon={<TbDeviceDesktop />}
            iconBg="bg-brand-50 dark:bg-brand-500/15"
            iconColor="text-brand-500 dark:text-brand-400" />
          <Metric title='Tersedia' amount={counts.available} icon={<TbDeviceDesktopCheck />}
            iconBg="bg-success-50 dark:bg-success-500/15"
            iconColor="text-success-600 dark:text-success-500" />
          <Metric title='Digunakan' amount={counts.in_use} icon={<TbDeviceDesktopCode />}
            iconBg="bg-warning-50 dark:bg-warning-500/15"
            iconColor="text-warning-600 dark:text-orange-400" />
          <Metric title='Perbaikan' amount={counts.maintenance} icon={<TbDeviceDesktopCog />}
        ...
        <ComponentCard title="Daftar perangkat">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Select placeholder="Tipe" options={typeOptions} onChange={setFilterType} className="dark:bg-dark-900"></Select>
            <Select placeholder="Status" options={statusOptions} onChange={setFilterStatus} className="dark:bg-dark-900"></Select>
        ...
        <ConfirmDialog
          ...
          onConfirm={() =>
            deleteDevice &&
            deleteMutation.mutate(deleteDevice)
          }
