...
export default function OwnerFnb() {
  ...
  const loadAll = async () => {
    ...
    if (!activeCat && catRes.data.data.length > 0)
      setActiveCat(catRes.data.data[0].id);
    setIsLoading(false);
  };
  ...
  const saveCat = async () => {
    ...
    try {
      if (catModal === "add")
        await fnbApi.createCategory({ name: catForm.name, is_active: true });
      else if (selCat)
        await fnbApi.updateCategory(selCat.id, { name: catForm.name });
      setCatModal(null);
      await loadAll();
    } finally {
      setSaving(false);
  ...
  return (
    <>
      ...
      <div className="gap-4 lg:flex items-start">
        ...
        <ComponentCard
          className="flex-1"
          title={filteredItems.length + " item pada kategori ini"}
          headerAction={
            <Button
              size="sm"
              onClick={() => {
          ...
          <div className="flex-1">
            {isLoading ? (
              ...
              <div className="rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
                <EmptyState
                  icon=""
                  title="Data tidak ditemukan untuk kategori ini"
                />
              </div>
            ) : (
      ...
      <div>
        ...
        <Modal
          ...
          title={catModal === "add" ? "Tambah Kategori" : "Edit Kategori"}
          size="sm"
        >
        ...
        <Modal
          ...
          title={itemModal === "add" ? "Tambah Item" : "Edit Item"}
        >
          <div className="space-y-4">
