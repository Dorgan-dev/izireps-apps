...
export function PlaySessionsPage() {
  ...
  const { data: sessions, isLoading } = useQuery({
    ...
    queryFn: () =>
      sessionsApi.list({ status: 'active,time_up' })
  ...
  const { data: rawBookings, isLoading: isBookingsLoading } = useQuery({
    ...
    queryFn: () => bookingsApi.list({ status: 'confirmed', }).then(r => r.data.data as Booking[]),
  ...
  const startBookingMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.response?.data?.message || 'Gagal memulai sesi dari booking');
    }
  });
    ...
    <><PageBreadcrumb items={[{ label: 'Sesi bermain', path: '/cashier/sessions' }]} pageDescription='Pelanggan yang sedang bermain akan tercatat dalam sesi' />
      <ComponentCard title='Daftar sesi aktif' headerAction={
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* List sesi */}
          {isLoading || isBookingsLoading ? (<Spinner className="py-16" />) : (
            <div className="flex flex-col gap-4">
              ...
              {waitings.length > 0 && (
                <div className="flex flex-col gap-3">
                  <p className="text-xs font-medium text-blue-600 uppercase tracking-wide">Menunggu Kedatangan (Hari Ini)</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {waitings.map(b => (
                      <WaitingBookingCard
                        key={b.id}
                        booking={b}
                        isStarting={startBookingMutation.isPending && startBookingMutation.variables === b.id}
