...
export default function CashierBookings() {
  ...
  const handleAction = async () => {
    ...
    try {
      if (actionModal.type === 'confirm') {
        ...
        await bookingsApi.reject(actionModal.booking.id, reason);
      }
      setActionModal(null);
      setReason('');
      load();
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-white mb-6">Booking Masuk</h1>
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl py-16 text-center">
          <p className="text-gray-500 text-sm">Tidak ada booking yang menunggu konfirmasi</p>
        </div>
      ) : (
        <div className="space-y-3">
          {bookings.map((b) => (
