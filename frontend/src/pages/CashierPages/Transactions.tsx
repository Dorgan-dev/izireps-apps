...
function ReceiptModal({ trx }: { trx: Transaction }) {
  return (
    <div className="flex flex-col gap-3 text-sm dark:text-gray-300">
      <div className="text-center pb-3 border-b border-gray-100 dark:border-gray-800">
        <p className="font-medium text-gray-900 text-base dark:text-white">Rental PlayStation</p>
      ...
      {(trx.items?.length ?? 0) > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 dark:text-gray-400">F&B</p>
          {trx.items!.map(item => (
            <div key={item.id} className="flex justify-between py-1 text-gray-700 dark:text-gray-300">
      ...
      <div className="border-t border-gray-100 dark:border-gray-800 pt-3 flex flex-col gap-1.5">
        ...
        {trx.payment_method && (
          <div className="flex justify-between text-gray-500 dark:text-gray-400 text-xs">
            ...
            <span>{paymentMethodLabel[trx.payment_method] || trx.payment_method}</span>
          </div>
        )}
        {trx.change_amount > 0 && (
          <div className="flex justify-between text-gray-500 dark:text-gray-400 text-xs">
...
export default function TransactionList() {
  ...
  const [dateFilter, setDate] = useState<string>(new Date().toISOString().slice(0, 10))
        ...
        headerAction={
          <div className="flex gap-3 flex-wrap items-center">
            <div className="w-32 sm:w-32">
              <Select className="w-32 sm:w-32" placeholder='Status' defaultValue='all'
                options={[
                ...
                value={statusFilter}
                onChange={e => { setStatus(e); setPage(1) }}
              />
            </div>
            <div className="w-32 sm:w-32">
              <DatePicker
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60 text-gray-700 dark:text-gray-300">
                      {transactions.map((trx) => (
                        <tr key={trx.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                          <td className="px-4 py-3 font-mono text-xs text-gray-500 dark:text-gray-400">{trx.invoice_number}</td>
                          <td className="px-4 py-3 text-xs">
                            {trx.paid_at ? formatDateTime(trx.paid_at) : '—'}
                          </td>
                          <td className="px-4 py-3 text-right font-medium">{formatRupiah(trx.gaming_total)}</td>
                          ...
                          <td className="px-4 py-3 text-center">
                            <Badge
                              ...
                              className={
                                trx.status === 'paid' ? 'bg-green-50 text-green-700 border border-green-200/60 dark:bg-green-500/10 dark:text-green-400 dark:border-green-500/20' :
                                  trx.status === 'pending' ? 'bg-amber-50 text-amber-700 border border-amber-200/60 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20' :
