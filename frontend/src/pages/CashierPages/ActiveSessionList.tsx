...
import { formatDuration, getElapsedMinutes, formatRupiah, calcGamingCost } from '../../utils'
...
// ─── Komponen utama ───────────────────────────────────────────────────────────
// ⚠️ SESUAIKAN:
//   - Path file: src/components/cashier/ActiveSessionList.tsx
//     (atau pindah ke src/pages/cashier/ sesuai struktur proyekmu)
//   - GET /api/sessions?status=active harus return relasi: device, customer
//   - navigate ke /kasir/sesi/:id — sesuaikan jika route berbeda di App.tsx
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700 flex items-center justify-between">
        <span><strong>{sessions.length} sesi</strong> sedang aktif</span>
        <span className="text-xs text-amber-500">Diperbarui setiap 10 detik</span>
      </div>
