      ...
      <ComponentCard title="Status Transaksi">
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-sm">
            Sesi bermain telah diselesaikan dan tagihan berhasil dicatat ke
            dalam sistem.
          </p>
          <div className="flex gap-4">
            ...
            <Button onClick={() => navigate("/cashier/sessions")}>
              Lihat Daftar Sesi
            </Button>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-5 border border-gray-100 dark:border-gray-800">
                ...
                {cart.length > 0 && (
                  <div className="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                    <p className="text-xs font-medium text-gray-500 uppercase mb-3">
                      Pesanan Tambahan (FnB)
                    </p>
                    <div className="space-y-2">
                      {cart.map((item) => (
                        <div
                          key={item.fnb_item_id}
                ...
                <div className="space-y-3">
                  <Row
                    label="Biaya Bermain"
                    value={formatRupiah(gamingTotal)}
                    note="Auto-kalkulasi"
                  />
              ...
              <div className="space-y-5">
                ...
                {paymentMethod === "cash" && (
                  <div className="pt-2 animate-in fade-in slide-in-from-top-2">
                    ...
                    {/* Alert Kembalian */}
                    <div
                      className={`mt-4 rounded-xl p-4 flex justify-between items-center transition-colors ${paid > remainingAmount ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-gray-50 dark:bg-gray-800/50"}`}
                    >
                      <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Kembalian
                      </span>
                      <span
                        className={`text-xl font-bold ${paid > remainingAmount ? "text-emerald-600 dark:text-emerald-400" : "text-gray-400 dark:text-gray-600"}`}
                      >
                        {formatRupiah(change)}
                      </span>
                    </div>
                ...
                {paymentMethod !== "cash" && (
                  <div className="mt-4 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-xl p-4">
                    <p className="text-sm text-blue-600 dark:text-blue-400">
                      Pastikan pelanggan telah melakukan transfer/scan sebesar{" "}
                      <strong>{formatRupiah(remainingAmount)}</strong> sebelum
                      menekan tombol Proses Pembayaran.
                    </p>
                  </div>
                )}
              </div>
  ...
  return (
    <div className="flex justify-between items-center text-sm">
      <span className="text-gray-500 dark:text-gray-400 flex items-center gap-2">
        {label}
        {note && (
          <span className="px-2 py-0.5 rounded-full bg-gray-200 dark:bg-gray-700 text-[10px] font-medium text-gray-500">
            {note}
          </span>
        )}
      </span>
      <span
        className={`${bold ? "font-bold" : "font-medium"} ${valueClass || "text-gray-900 dark:text-white"}`}
      >
        {value}
      </span>
    </div>
