...
function slotPosition(start, end) {
  const ORIGIN = 8 * 60;
  const RANGE = 16 * 60;
...
export default function DeviceSchedule() {
  ...
  return (
    <>
      <PageMeta
        title="IZIReps | Device Schedule"
        description="Jadwal penggunaan perangkat secara realtime"
      />
      <PageBreadcrumb
        items={[{ label: "Device Schedule", path: "/device-schedule" }]}
      />
            ...
            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
                ...
                <div className="flex flex-1 overflow-x-auto">
                  <div className="relative flex min-w-[640px] flex-1">
                    {HOURS.map((h) => (
                      <div
                        key={h}
                        className="flex-1 border-r border-gray-100 py-3 text-center dark:border-gray-800/60 last:border-r-0"
                      >
              ...
              {devices.length === 0 ? (
                ...
                devices.map((device) => {
                  ...
                  const statusCfg =
                    STATUS_CONFIG[device.status] ?? STATUS_CONFIG.available;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-40 shrink-0 border-r border-gray-100 dark:border-gray-800 px-4 py-4 flex flex-col justify-center gap-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-medium ${statusCfg.badge}`}
                            >
                              <span
                      ...
                      <div className="flex-1 overflow-x-auto">
                        <div
                          className="relative min-w-[640px] h-full"
                        ...
                        >
                          ...
                          <div className="absolute inset-0 flex pointer-events-none">
                            {HOURS.map((h) => (
                              <div
                                key={h}
                                className="flex-1 border-r border-gray-100 dark:border-gray-800/60 last:border-r-0"
                              />
                            ))}
                          </div>
                          ...
                          {isSameDay(selectedDate, today) &&
                            (() => {
                              ...
                              if (
                                ...
                                return (
                                  <div
                                    className="absolute top-0 bottom-0 z-20 w-0.5 bg-red-400"
                                    style={{ left: `${left}%` }}
                                  >
                          ...
                          {slots.map((slot) => {
                            ...
                            const slotColor =
                              SLOT_COLOR[slot.type] ?? "bg-gray-400";
                            return (
                              <div
                                key={slot.id}
              ...
              {[
                ...
                },
                {
                  ...
                  value: devices.filter(
                    (d) => d.status === "in_use" || d.status === "booked",
                  ).length,
                  color: "text-amber-600 dark:text-amber-400",
                },
                {
                  label: "Total Jadwal",
                  value: Object.values(schedules).reduce(
                    (acc, s) => acc + s.length,
                    0,
                  ),
